...
void cityBlock(pcl::visualization::PCLVisualizer::Ptr& viewer, ProcessPointClouds<pcl::PointXYZI>* pointProcessorI, const pcl::PointCloud<pcl::PointXYZI>::Ptr& inputCloud)
{
	...
	/*Filter: Crop the scene to requested dimensions and remove the points from the roof top*/
	filterCloud = pointProcessorI->FilterCloud(inputCloud, 0.01f, Eigen::Vector4f (-30, -20, -20, -1), Eigen::Vector4f ( 40, 27, 15, 5));
    //filterCloud = pointProcessorI->FilterCloud(inputCloud, 0.15 , Eigen::Vector4f (-20, -6, -2, 1), Eigen::Vector4f ( 30, 7, 5, 1));
	/*Segmentation: Segment the scene to create to clouds one for road and another for objects.*/
	std::pair<pcl::PointCloud<pcl::PointXYZI>::Ptr, pcl::PointCloud<pcl::PointXYZI>::Ptr> segmentCloud = pointProcessorI->SegmentPlane_RANSAC(filterCloud, 50, 0.3);
...
int main (int argc, char** argv)
{
    ...
    std::vector<std::__fs::filesystem::path> stream = pointProcessorI->streamPcd("../src/sensors/data/pcd/data_4");
    ...
    while (!viewer->wasStopped ())
    {
      ...
      //inputCloudI = pointProcessorI->loadPcd(("../src/sensors/data/pcd/data_3/0831.pcd"));
